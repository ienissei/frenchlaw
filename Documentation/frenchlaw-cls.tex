...
\begin{developer}
Il est possible de modifier le comportement de l'option grâce à \cs{if@landscape}{Opérateur conditionnel attaché à l'option |landscape|.}.
\end{developer}
...
\begin{developer}
Il est possible de modifier le comportement de l'option grâce à \cs{if@marginalia}{Opérateur conditionnel attaché à l'option |marginalia|.}.
\end{developer}
...
\begin{developer}
Il est possible de modifier le comportement des deux options grâce à \cs{if@sansserif}{Opérateur conditionnel attaché aux options |sans-serif| et (si la condition n'est pas remplie) |serif|.}.
\end{developer}
...
\begin{developer}
Le nom du style sélectionné est passé dans la macro interne \cs{@biblatexstyle}{Macro interne stockant le nom du style |biblatex| sélectionné.}.
\end{developer}
...
% Define a command to reset colours
...
\subparagraph*{<title>}% Sous-paragraphe
\end{macro}
...
\makeparastyle{<nom du style>}{ % Création du style
 % Pour la commande \para
  ...
  \renewcommand{\paranumcolour}{\if@colour\maincolour\fi}
...
\makeprecisstyle{default}{
	...
	\renewcommand{\cftprecisnumstyle}{\if@modern\if@colourall\maincolour\fi\fi}
...
\begin{developer}
Pour la mise en œuvre des styles de table des matières, qui peuvent requérir des manimulations sur le contenu des variables |\cftXname|, la commande \cs{maketoclabel}{Commande-outil contenant seulement l'étiquette des noms de chapitres, sections, etc. pour le paramétrage de |babel|, susceptible d'être redéfinie par les styles de table des matières.}~-- qui ne fait rien~-- est ajoutée dans la définition de |\cftXname| et destinée à être remplacée par une commande contenant des instructions de formattage. \frenchlaw définit deux commandes à cet effet: \cs{toclabelnobox}{Commande utilisée par les styles de tables des matières pour formatter les noms des subdivisions principales de manière classique.}, utilisée pour composer simplement le nom de la subdivision, et \cs{toclabelbox}{Commande utilisée par les styles de tables des matières pour formatter les noms des subdivisions principales dans une boîte de largeur fixe.}, pour composer les noms des subdivisions dans des boîtes de largeur fixe. On pourra, par exemple, utiliser:
\par\vspace{-\baselineskip}
\begin{macro}
% Dans les définitions de <babel>:
\def\cftchaptername{\maketoclabel{Chapitre}}
% Dans le style de table des matières:
\let\maketoclabel=\toclabelbox
\end{macro} 
\end{developer}

\begin{noprint}
%	Create the \maketoclabel placeholder command
...
%	Create a command that formats ToC labels for general use
...
\begin{noprint}
%	Create the <basic> ToC style
\maketocstyle{basic}{
	%	Redefine \maketoclabel to \toclabelnobox (format ToC labels)
	\let\maketoclabel=\toclabelnobox
	%	Set up ToC components indentation
	...
	\cftsetindents{section}{\if@article\z@\else 1em \fi}{\tocnumwidth}
	\cftsetindents{subsection}{\if@article 1em \else 2em \fi}{\tocnumwidth}
	\cftsetindents{subsubsection}{\if@article 2em \else 3em \fi}{\tocnumwidth}
	\cftsetindents{paragraph}{\if@article 3em \else 4em \fi}{\tocnumwidth}
	\cftsetindents{subparagraph}{\if@article 4em \else 5em \fi}{\tocnumwidth}
}
\end{noprint}
...
\begin{noprint}
%	Create the frenchlaw> ToC style
\maketocstyle{frenchlaw}{
	%	Redefine \maketoclabel to \toclabelbox (format ToC labels into a box)
	\let\maketoclabel=\toclabelbox
	...
	\cftsetindents{subsection}{\if@article\tocnumwidth\else\z@\fi}{\tocnumwidth}
	\cftsetindents{subsubsection}{%
		\if@article 2\tocnumwidth\else\toclabelwidth\fi}{\if@article.5\tocnumwidth\else\tocnumwidth\fi}
	...
	\cftsetindents{subparagraph}{%
		...
		{\if@article.75\tocnumwidth\else.5\tocnumwidth\fi}
}
\tocstyle{frenchlaw}
\end{noprint}

% TODO > Create new ToC styles?
...
\begin{noprint}
%	Correct an old LaTeX 2.09 bug: add proper \addvspace before chapters (and heading) content lines, so the first ToC entry does not drop one or two lines below the start of normal chapters
...
% TODO > Récupérer outline / summary et les intégrer dans les méthodes ToC supra + dans les styles de précis, e.g. italiques ou autres
